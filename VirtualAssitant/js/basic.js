...
//   })
  ...
  let url = "http://127.0.0.1:8000/generate_response_azure"
  let body = JSON.stringify({
...
function scrollToBottom(elementId) {
  ...
  function speakAndFetchResponse() {
    ...
    if(userQuery === ''){
      console.log("No user query detected, not sending request");
      return "you are not audible. can you speak louder";
    }
...
window.startSession = () => {
    ...
    function startSpeechRecognition() {
      let speechConfig = SpeechSDK.SpeechConfig.fromSubscription(subscriptionKey, cogSvcRegion);
      let audioConfig = SpeechSDK.AudioConfig.fromDefaultMicrophoneInput();
      recognizer = new SpeechSDK.SpeechRecognizer(speechConfig, audioConfig);
      let finalTranscript = ''; // Global variable to store final transcript
      ...
      recognizer.recognized = (s, e) => {  
        if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {  
          finalTranscript += ' ' + e.result.text; // appending final transcript with complete recognized text  
        }  
        console.log(`RECOGNIZED: Text=${finalTranscript}`);  
        document.getElementById('openAIQuery').value = finalTranscript.trim(); // updating the text field with final transcript  
      };
